...
from paxos.acceptor import finalise_prepare, finalise_accept
import paxos.state
from paxos.proposer import propose
...
ORDER_REPLICAS = [
    "http://order_service_1:6001",
    "http://order_service_2:6002",
    "http://order_service_3:6003"
]
    ...
    def do_POST(self):
        ...
        if self.path == "/set_leader":
            ...
            try:
                ...
                paxos.state.proposal_counter += 1
                proposal_num = paxos.state.proposal_counter
                ...
                print(f"[PAXOS] Starting proposal {proposal_num} for txn {txn_id}", flush=True)

                success = propose(txn_id, value, ORDER_REPLICAS, proposal_num)
                if not success:
                    self._send_json_response(503, {"error": {"message": "Consensus not reached"}})
                    return
                ...
                print("[PAXOS] Update to catalog successful", flush=True)
                # if order is successful, write to in memory storage first
                # Commit locally via learner
                with order_lock:
                    global TRANSACTION_ID
                    TRANSACTION_ID += 1
                    orders_data[TRANSACTION_ID] = value
                    learn(txn_id, value) 
                ...
                print("[PAXOS] update propagation successful", flush=True)
            ...
            body = json.loads(self.rfile.read(content_length).decode())
            response = finalise_prepare(body["txn_id"], body["proposal_num"])
            self._send_json_response(200, response)
            return

        elif self.path == "/accept":
            body = json.loads(self.rfile.read(content_length).decode())
            response = finalise_accept(body["txn_id"], body["proposal_num"], body["value"])
            self._send_json_response(200, response)
            return
